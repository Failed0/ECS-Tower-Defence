 ...
 */
    ...
    public int defence(Building b) {
        ...
        if (b.getAllBugs().length != 0) {
            ...
            if (count % delay == 0) {
                System.out.println("(Students)Setting Up an antivirus with a chance of instantly removing the bug.");
                if (level <= 30) {
                    if (Math.random() <= (level * 0.01) + 0.2 || b.getAllBugs().length != 0) {
                        ...
                        System.out.println("(Students)No bugs in the building to attack");
                    }
                }
                if (level > 30) {
